...
class EpsonEposPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    private fun onConnect(@NonNull call: MethodCall, @NonNull result: Result) {
        ...
        var resp = EpsonEposPrinterResult("onConnect${type}", false)
        try {
            if (connectPrinter(target, series)) {
                resp.success = true
                resp.message = "Can connect to the printer."
                result.success(resp.toJSON())
                Log.e("logTag", "Can ConnectPrinter $resp")
                if (mPrinter != null) {
                    mPrinter!!.clearCommandBuffer()
                }
            }
